...
# ============= 数据迁移接口 =============
@app.route('/api/migrate-from-old')
def migrate_from_old():
    """从旧环境迁移数据到新环境"""
    import tcb_config as cfg
    old_env = "personnel-api-d0gsohasr28067fae"
    new_env = cfg.ENV_ID
    
    results = {}
    
    for collection in ["personnel", "users", "transfers", "leave_records", "trip_records"]:
        # 从旧环境读取
        cfg.ENV_ID = old_env
        try:
            old_data = tcb_query(collection, limit=500)
        except Exception as e:
            results[collection] = f"读取失败: {e}"
            continue
        
        if not old_data:
            results[collection] = "旧环境无数据"
            cfg.ENV_ID = new_env
            continue
        ...
        results[collection] = f"成功 {success}, 失败 {fail}"
