...
def update_person(person_id):
    ...
    try:
        ...
        fields = []
        values = []
        for key in ['name','gender','id_card','birth','edu','hometown','position','dept','project','phone','cert','category','salary','status','status_detail','hire_date','leave_date','school','major','remark','is_external']:
            if key in data:
                val = data[key]
                # salary空字符串转None
...
def export_data():
    ...
    try:
        ...
        def get_row_data(p, idx, export_type):
            ...
            if export_type in ('all', 'regular', 'external', 'huamingce'):
                return [
                    idx + 1,
                    p['name'],
                    p.get('gender', ''),
        ...
        if export_type in ('huamingce', 'all', 'regular', 'external', 'project', 'dept'):
            ...
            def write_sheet(ws, title, groups, group_order, headers, styles):
                ...
                title_cell = ws.cell(row=1, column=1, value=f'中铁建工集团西南分公司安装公司{title}')
                ...
                for dept in group_order:
                    ...
                    # 领导优先
                    leaders = [p for p in group if is_leader(p.get('position'))]
                    others = [p for p in group if not is_leader(p.get('position'))]

                    for p in leaders + others:
                        row_data = get_row_data(p, seq - 1, export_type)
                        row_data[0] = seq
            ...
            data_font = Font(name='微软雅黑', size=10)
            dept_fill = PatternFill(start_color='E2EFDA', end_color='E2EFDA', fill_type='solid')
            leader_fill = PatternFill(start_color='FFF2CC', end_color='FFF2CC', fill_type='solid')
            ...
            # 按项目分组时只包含项目部人员，按后台分组时包含后台人员+预算/供应链/其他
            if group_key == 'project':
                ...
                def is_backend_or_support(p):
                    ...
                    dept = str(p.get('dept') or '')
                    # 后台人员
                    if project in ['后台', '未分配']:
                        return True
                    # 预算/供应链/其他人员
                    if any(k in position or k in dept for k in ['预算', '供应链', '其他']):
                        return True
                    return False
                filtered_people = [p for p in people if is_backend_or_support(p)]
            else:
...
# ============= 数据迁移接口 =============
@app.route('/api/migrate-to-tcb')
def migrate_to_tcb():
    """从Supabase迁移数据到微信云开发数据库"""
    ...
    tcb_env = "personnel-api-d0gsohasr28067fae"
    tcb_url = "https://api.weixin.qq.com/tcb/databaseadd"
    ...
    try:
        ...
        for row in rows:
            ...
            query = 'db.collection("personnel").add({data:' + _json.dumps(data, ensure_ascii=False) + '})'
            payload = {"env": tcb_env, "query": query}
            r = req.post(tcb_url, headers=headers, json=payload, timeout=15)
            result = r.json()
            if result.get("errcode", 0) == 0:
                ...
                fail += 1
                if fail <= 2:
                    results[f"personnel_error_{fail}"] = result
        
        results["personnel"] = f"成功 {success}, 失败 {fail}"
        cur.close()
        conn.close()
    except Exception as e:
